...
// FIXED: Swapped GNews URL out for TMDB Upcoming Announcements endpoint
const NEWS_URL = "https://api.themoviedb.org/3/movie/upcoming?language=en-US&page=1";

// UI Helper state reset to clean views when switching context away from News
function clearNewsView() {
...
function updateBanner(movie) {
    ...
    setTimeout(() => {
        ...
        bannerRating.textContent = `⭐ ${movie.vote_average.toFixed(1)}`;
...
function showSavedMovies(movieIds, title) {
    ...
    if (movieIds.length === 0) {
        moviesContainer.innerHTML = "<h2>No movies saved</h2>";
        return;
    }
...
function renderMovies(movies) {
    ...
    document.querySelectorAll(".movie-card").forEach(card => {
        card.addEventListener("click", (e) => {
            if (e.target.closest(".favorite-btn") || e.target.closest(".watchlist-btn")) {
                return;
            }

            popupPoster.src = card.dataset.poster;
            const year = card.dataset.release ? card.dataset.release.split("-")[0] : "Unknown";

            popupTitle.textContent = `${card.dataset.title} (${year})`;
            ...
            fetch(`https://api.themoviedb.org/3/movie/${card.dataset.id}?language=en-US`, OPTIONS)
                .then(response => response.json())
                .then(data => {
                    popupTagline.textContent = data.tagline || "No tagline available";
                    
                    fetch(`https://api.themoviedb.org/3/movie/${card.dataset.id}/watch/providers`, OPTIONS)
                        .then(response => response.json())
                        .then(providerData => {
                            ...
                            if (providers.length > 0) {
                                ...
                                providersList.innerHTML = providers.map(provider => `
                                    <a href="${providerLinks[provider.provider_name] || '#'}" target="_blank" class="provider">
                                        <img src="https://image.tmdb.org/t/p/w92${provider.logo_path}" alt="${provider.provider_name}" title="${provider.provider_name}">
                                        <span>${provider.provider_name}</span>
                                    </a>
                                `).join('');
                            } else {
                                providersList.innerHTML = "<p>Currently unavailable in India 🇮🇳</p>";
                            }
                        });
                    ...
                    fetch(`${MOVIE_CREDITS_URL}${card.dataset.id}/credits`, OPTIONS)
                        .then(response => response.json())
                        .then(castData => {
                            popupCast.innerHTML = castData.cast.slice(0, 6).map(actor => {
                                const photo = actor.profile_path ? `https://image.tmdb.org/t/p/w185${actor.profile_path}` : 'https://via.placeholder.com/120x180?text=No+Photo';
                                return `
...
watchTrailerBtn.addEventListener("click", () => {
    if (trailerUrl) {
        let recentTrailers = JSON.parse(localStorage.getItem("recentTrailers")) || [];
        const movieData = {
        ...
        recentTrailers = recentTrailers.filter(movie => movie.id !== movieData.id);
        recentTrailers.unshift(movieData);
        recentTrailers = recentTrailers.slice(0, 10);

        localStorage.setItem("recentTrailers", JSON.stringify(recentTrailers));
    ...
    if (favoriteBtn) {
        ...
        if (favoriteBtn.classList.contains("active")) {
            if (!favorites.includes(movieId)) {
                ...
                fetch("https://moviehub-z5xt.onrender.com/favorites/add", {
                    method: "POST",
                    headers: {
...
favoritesNav.addEventListener("click", () => {
    ...
    backBtn.style.display = "block";
    document.querySelectorAll(".top-icons i").forEach(icon => icon.classList.remove("active"));
    favoritesNav.classList.add("active");
    showSavedMovies(favorites, "FAVORITES");
});
...
watchlistNav.addEventListener("click", () => {
    ...
    showSavedMovies(watchlist, "watchlist");
});
...
setInterval(() => {
    ...
    updateBanner(bannerMovies[currentBanner]);
}, 5000);
...
// FIXED: Cleaned and updated to render production releases from TMDB
function fetchNews() {
    newsContainer.style.display = "grid";
    moviesContainer.style.display = "none";

    fetch(NEWS_URL, OPTIONS) // Included your working TMDB auth header payload object options
        .then(response => response.json())
        .then(data => {
            newsContainer.innerHTML = data.results.slice(0, 12).map(movie => {
                const newsImg = movie.backdrop_path 
                    ? `https://image.tmdb.org/t/p/w780${movie.backdrop_path}`
                    : 'https://via.placeholder.com/400x220?text=MovieHub+Announcement';
                
                const releaseDate = movie.release_date || 'Coming Soon';

                return `
                    <div class="news-card">
                        <img src="${newsImg}" alt="${movie.title}">
                        <h3>📢 Announcement: ${movie.title}</h3>
                        <p><strong>Expected Release:</strong> ${releaseDate}<br><br>${movie.overview || 'No extended summary outline details available yet.'}</p>
                        <a href="#" class="news-action-btn" data-id="${movie.id}">View Details</a>
                    </div>
                `;
            }).join('');

            // Hook up click listener for details inside the news view card elements
            document.querySelectorAll(".news-action-btn").forEach(btn => {
                btn.addEventListener("click", (e) => {
                    e.preventDefault();
                    const mockCard = { dataset: { id: btn.dataset.id } };
                    // Reuses your existing click modal popup structure logic!
                    const targetCard = document.querySelector(`.movie-card[data-id="${btn.dataset.id}"]`);
                    if (targetCard) {
                        targetCard.click();
                    } else {
                        // Fallback manual trigger setup if card element hasn't updated to view
                        popupPoster.src = e.target.closest('.news-card').querySelector('img').src;
                        popupTitle.textContent = e.target.closest('.news-card').querySelector('h3').textContent.replace('📢 Announcement: ', '');
                        popupOverview.textContent = e.target.closest('.news-card').querySelector('p').textContent;
                        moviePopup.style.display = "flex";
                    }
                });
            });
        ...
        .catch(error => {
            console.error("TMDB News View Generation Fail:", error);
            newsContainer.innerHTML = "<p>Unable to retrieve trending movie updates right now.</p>";
        });
}
...
newsNav.addEventListener("click", () => {
    ...
    genresContainer.style.display = "none";
    backBtn.style.display = "block";
    sectionTitle.textContent = "ENTERTAINMENT NEWS";
    fetchNews();
});